---
title: "Offline testing via SDK"
sidebarTitle: "Offline testing via SDK"
---
...
def test_paris_trip():
    ...
    output = response.choices[0].message.content
    includes_eiffel_tower = baserun.evals.includes("includes_eiffel_tower", output, ["Eiffel Tower"])

    # Optional:The test will fail if the eval failed
    assert includes_eiffel_tower
```
...
describe("end-to-end tests", () => {
  ...
  it("should suggest the Eiffel Tower", async () => {
    ...
    const includes_eiffel_tower = baserun.evals.includes("includes_eiffel_tower", output, ["Eiffel Tower"]);

    # Optional: The test will fail if the eval failed
    expect(includes_eiffel_tower).toBe(true);
  });
});
```

...
Tags are key-value pairs that can be used to add extra information to a completion or a trace. Baserun provides a set of predefined tags for common use cases, and you can also create custom tags.

```python

Much like the completion object, these functions can be called on the trace object to add tags to the trace. 
...
<Accordion title="client.log(message: str, name: Optional[str] = None, metadata: Optional[Dict] = None): Tag">
  ...
  <ParamField path="name" type="string">
    An optional name that will show up in the trace detail view in the UI
  </ParamField>
  <ParamField path="metadata" type="object" />
  **Returns**
  <ResponseField type="Tag" name="tag" />

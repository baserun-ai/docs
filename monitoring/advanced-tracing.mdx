...
```typescript typescript
import { AssignMetadataFunc, baserun } from "baserun"

async function askQuestion(question: string = "What is the capital of the US?"): Promise<string> {
  const workflow = baserun.trace(
    // You can add additional last parameter in the callback to get a function for setting trace metadata
    async (fn: AssignMetadataFunc) => {
      // Make a completion
      ...
      // Your code here, for example:
      const answerId = await persistAnswer(question, completion);

      // Add whatever metadata you like
      fn({ answer_id: answerId });
